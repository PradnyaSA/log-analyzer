...
### Problem: 
I woke up to a Priority 2 incident last week — our concierge agent had a 20% spike in "information not found" responses. Typical SLA to resolving a Priority 2 incident is ~4-8hours, with initial response due in ~15mins to avoid further escalations. Call-To-Action: Triaging that can pin down the exact log line, tool call, and step in the chain responsible — root cause analysis starting with logs, with every claim traceable back to the exact line that proves it.

### User: 
Any engineer who invokes AI agent with a read-only, temporary access to our application/server log files and ADK agent audit log store via the CLI during incident triage.
